...
There are two sets of stock data files in `bin/wiki_data/`, each with its own
refresh script:
...
Both scripts are pure Python 3 stdlib — no extra dependencies beyond what
`nwn-wiki` already requires. Commit the regenerated JSON after running either.
